...
const Navbar: React.FC<NavbarProps> = ({ onGoToMain, onSearchResults }) => {
  ...
  // Filtrar búsquedas recientes basado en el término de búsqueda
  ...
  const handleSearch = (term: string) => {
    ...
    const results = imageDatabase[searchTermLower] || [];
    
    console.log('Buscando:', term, 'Resultados:', results);
    
    // Pasar los resultados al componente padre si existe la prop
    if (onSearchResults) {
      onSearchResults(results, term);
    }
    
    // También puedes manejar los resultados directamente aquí
    // o pasarlos a otro componente
  };
  ...
  const goBack = () => {
    ...
    setCurrentSearch('');
    setSearchTerm('');
    // También notificar al componente padre para que limpie los resultados
    if (onSearchResults) {
      onSearchResults([], '');
    }
  };
  ...
  // Si estamos mostrando resultados, cambiamos completamente la navbar
  if (showResults) {
    return (
